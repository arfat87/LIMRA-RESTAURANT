...
for (const file of allFiles) {
  if (file.endsWith('.js') || file.endsWith('.html') || file.endsWith('.vue') || file.endsWith('.ts') || file.endsWith('.css') || file.endsWith('.json')) {
    ...
    if (content.includes('Menara Bibi') || content.includes('menara bibi') || content.includes('Menara') || content.includes('menara')) {
      console.log(`Found match in: ${file}`);
      const lines = content.split('\n');
      lines.forEach((line, idx) => {
        if (line.includes('Menara') || line.includes('menara')) {
