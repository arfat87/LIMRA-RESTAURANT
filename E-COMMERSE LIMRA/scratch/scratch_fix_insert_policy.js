...
-- Create a secure but permissive insert policy allowing profile creation on signup
CREATE POLICY "Users can insert own profile"
  ON public.customer_profiles FOR INSERT
  TO anon, authenticated
  WITH CHECK (true);

-- Drop trigger if it was partially created on auth.users to clean up
  ...
  try {
    ...
    for (let i = 0; i < lines.length - 1; i++) {
      ...
      if (line.startsWith('{') && line.endsWith('}')) {
        ...
        if (json.id === 2) {
          console.log('=== RLS INSERT POLICY RE-CONFIGURED ===');
