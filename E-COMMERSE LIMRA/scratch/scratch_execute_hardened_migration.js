...
const sqlContent = fs.readFileSync('migrations/20260604233000_advisor_security_hardened.sql', 'utf8');
  ...
  try {
    ...
    for (let i = 0; i < lines.length - 1; i++) {
      ...
      if (line.startsWith('{') && line.endsWith('}')) {
        ...
        if (json.id === 5) {
          console.log('=== HARDENED MIGRATION EXECUTION RESULT ===');
...
proc.stderr.on('data', (data) => {
  ...
  if (errStr.includes('Insforge MCP server started')) {
    setTimeout(() => {
      console.log('Sending hardened migration SQL...');
