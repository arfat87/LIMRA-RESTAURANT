import { insforge, saveOrder } from '../lib/insforge.js';
...
// ====================================================
let selectedCategory = 'biryani'; // Default category to show
...
function renderCategoryChips() {
  ...
  container.innerHTML = allChipHtml + chipsHtml;
...
function setupCartUI() {
  ...
  $('#checkout-form').addEventListener('submit', async e => {
    ...
    const payment = fd.get('payment_method')?.toString() || 'cash';
    ...
    try {
      // 1. Prepare notes format: [TABLE: X] | [PAYMENT: cash] | [PAYMENT_STATUS: PENDING] | Instruction
      const paymentNote = `[PAYMENT: ${payment}] | [PAYMENT_STATUS: PENDING]`;
      const combinedNotes = [`[TABLE: ${currentTable}]`, paymentNote, instruction].filter(Boolean).join(' | ');

      const zone = currentTable <= 9 ? 'indoor' : 'outdoor';

      // 2. Prepare items format for saveOrder RPC
