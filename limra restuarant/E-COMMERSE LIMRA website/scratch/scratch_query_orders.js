...
const query = `
SELECT id, order_number, customer_name, customer_phone, total_amount, status FROM public.orders ORDER BY created_at DESC LIMIT 5;
`;
  ...
  try {
    ...
    for (let i = 0; i < lines.length - 1; i++) {
      ...
      if (line.startsWith('{') && line.endsWith('}')) {
        ...
        if (json.id === 2) {
          console.log('=== ORDERS SELECT RESULT ===');
